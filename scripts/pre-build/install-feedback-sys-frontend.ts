...
const CSS_SOURCE_FILE =
  "../node_modules/oiwiki-feedback-sys-frontend/dist/style.css";
...
console.log(
  `Copying oiwiki-feedback-sys-frontend script to ${JS_TARGET_FILE}...`,
);
...
console.log(
  `Copying oiwiki-feedback-sys-frontend css to ${CSS_TARGET_FILE}...`,
);
...
console.log(
  `Injecting variables to oiwiki-feedback-sys-frontend content script ${CONTENT_SCRIPT_TARGET_FILE}...`,
);
...
await fs.promises.writeFile(
  ...
  (await fs.promises.readFile(mkdocsConfigFile, "utf-8"))
    .replace(
      "{OIWikiFeedbackSystemFrontendJS}",
      path.relative(docsDir, jsDestFile) + "?v=" + jsFileMd5,
    )
    .replace(
      "{OIWikiFeedbackSystemFrontendCSS}",
      path.relative(docsDir, cssDestFile) + "?v=" + cssFileMd5,
    )
    .replace(
